...
void IndoorNavigationService::SetupRoutes() {
    ...
    Pistache::Rest::Routes::Post(
        router_, "/set_rss/:device_id/:ssid1/:rssi1/:ssid2/:rssi2/:ssid3/:rssi3/:ssid4/:rssi4/:end?",
        Pistache::Rest::Routes::bind(&IndoorNavigationService::SetReceivedSignalStrengths, this));
    Pistache::Rest::Routes::Post(
        router_, "/reset_device_location/:device_id",
        Pistache::Rest::Routes::bind(&IndoorNavigationService::ResetDeviceLocation, this));
    Pistache::Rest::Routes::Get(router_, "/get_device_position/:device_id",
                                Pistache::Rest::Routes::bind(&IndoorNavigationService::GetDevicePosition, this));
    Pistache::Rest::Routes::Get(router_, "/get_employee_position/:employee_id",
                                Pistache::Rest::Routes::bind(&IndoorNavigationService::GetEmployeePosition, this));
    Pistache::Rest::Routes::Get(router_, "/ready",
                                Pistache::Rest::Routes::bind(&IndoorNavigationService::HandleReady, this));
    Pistache::Rest::Routes::Get(router_, "/auth",
                                Pistache::Rest::Routes::bind(&IndoorNavigationService::Auth, this));
...
void IndoorNavigationService::SetReceivedSignalStrengths(const Pistache::Rest::Request &request,
    ...
    std::vector<std::string> ssid_list = {
        request.param(":ssid1").as<std::string>(), request.param(":ssid2").as<std::string>(),
        request.param(":ssid3").as<std::string>(), request.param(":ssid4").as<std::string>()};

    std::vector<double> rssi_list = {request.param(":rssi1").as<double>(), request.param(":rssi1").as<double>(),
                                     request.param(":rssi1").as<double>(), request.param(":rssi1").as<double>()};

    console_->debug("ssids: {0}, {1}, {2}, {3}", ssid_list[0], ssid_list[1], ssid_list[2], ssid_list[3]);
    console_->debug("rssis: {:03.3}, {:03.3}, {:03.3}, {:03.3}", rssi_list[0], rssi_list[1], rssi_list[2],
                    rssi_list[3]);
...
void IndoorNavigationService::Auth(const Pistache::Rest::Request &request,
                                   Pistache::Http::ResponseWriter response) {
