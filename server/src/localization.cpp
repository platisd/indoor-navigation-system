...
wifiParams_t * Localization::FillNodeDataPoints(wifiParams_t *  wifiNodeBlock, AccessPointRssiListPair mac_rssi_)
{
	initKalmanParams(wifiNodeBlock);

	wifiNodeBlock->macAddress = (char *) mac_rssi_.first.mac_addr.c_str();

	wifiNodeBlock->noSampleData = mac_rssi_.second.size();

	for (int i = 0; i < mac_rssi_.second.size(); ++i)
	{
		wifiNodeBlock->rssisampledata[(i%NUMBER_SAMPLES)] = (float)mac_rssi_.second[i];
	}
...
insNode_t * Localization::FillNodesDataPoints(const char * device_id, std::vector<AccessPointRssiListPair> mac_rssi_list)
{
	insNode_t * insNode;
	...
	//std::string msg = "Number of distinct mac addresses:" + std::to_string(mac_rssi_list.size());
	//console_->debug(msg);
...
Position Localization::ProcessRSSIDataSet(const std::string& device_id)
{
	...
	std::vector<AccessPoint> distn = data_store_->GetDistinctAccessPoints(device_id);

	if (distn.size() >= TRILATERAT_NUMBER_NODES)
	{
		std::vector<AccessPointRssiListPair> mac_rssi_list = data_store_->GetRSSISeriesData(device_id, distn);
